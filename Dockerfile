...
# Setup musl
RUN DIGEST='7d5b0b6062521e4627e099e4c9dc8248d32a30285e959b7eecaa780cf8cfd4a4' \
 && wget https://www.musl-libc.org/releases/musl-1.2.3.tar.gz \
 && echo "${DIGEST} musl-1.2.3.tar.gz" | sha256sum -c - \
...
RUN OSX_SDK_PATH=https://s3.dockerproject.org/darwin/v2/$OSX_SDK.tar.xz \
 && DIGEST='694a66095a3514328e970b14978dc78c0f4d170e590fa7b2c3d3674b75f0b713' \
 && wget $OSX_SDK_PATH \
 && echo "${DIGEST} ${OSX_SDK}.tar.xz" | sha256sum -c - \
...
RUN DIGEST='199d8fa4523c248d1ee49bf300da031e6a56aab5ec7261927e0a8bdfe0737bf4' \
 && wget https://github.com/karalabe/xgo/blob/master/docker/base/patch.tar.xz?raw=true -O patch.tar.xz \
 && echo "${DIGEST} patch.tar.xz" | sha256sum -c - \
...
RUN mkdir -p /freebsd/x86_64-pc-freebsd12 && cd /freebsd \
 && DIGEST='e85b256930a2fbc04b80334106afecba0f11e52e32ffa197a88d7319cf059840' \
 && wget http://ftp-archive.freebsd.org/pub/FreeBSD-Archive/old-releases/amd64/12.3-RELEASE/base.txz \
 && echo "${DIGEST} base.txz" | sha256sum -c - \
 && DIGEST='8d7425ddbbf7b99fad26f439723eaf80df6509cc2d70f871d5bbb6e721055323' \
 && wget http://ftp-archive.freebsd.org/pub/FreeBSD-Archive/old-releases/amd64/12.3-RELEASE/lib32.txz \
 && echo "${DIGEST} lib32.txz" | sha256sum -c - \
...
RUN DIGEST='82a40a37b13a12facb36ac7e87846475a1d80f2e63467b1b8d63ec8b6a2b63fc' \
 && wget http://ftp.gnu.org/gnu/binutils/binutils-2.25.1.tar.gz \
 && echo "${DIGEST} binutils-2.25.1.tar.gz" | sha256sum -c - \
 ...
 && popd \
 && rm -rf binutils*
RUN DIGEST='9156d32edac6955bc53b0218f5f3763facb890b73a835d5e1b901dcf8eb8b764' \
 && wget http://ftp.gnu.org/gnu/gmp/gmp-6.0.0a.tar.xz \
 && echo "${DIGEST} gmp-6.0.0a.tar.xz" | sha256sum -c - \
 && tar -xf gmp*.tar.xz \
 && rm gmp*.tar.xz \
 && pushd gmp* \
 ...
 && popd \
 && rm -rf gmp*
RUN DIGEST='6835a08bd992c8257641791e9a6a2b35b02336c8de26d0a8577953747e514a16' \
 && wget http://ftp.gnu.org/gnu/mpfr/mpfr-3.1.3.tar.xz \
 && echo "${DIGEST} mpfr-3.1.3.tar.xz" | sha256sum -c - \
 && tar -xf mpfr*.tar.xz \
 && rm mpfr*.tar.xz \
 && pushd mpfr* \
 ...
 && popd \
 && rm -rf mpfr*
RUN DIGEST='617decc6ea09889fb08ede330917a00b16809b8db88c29c31bfbb49cbf88ecc3' \
 && wget http://ftp.gnu.org/gnu/mpc/mpc-1.0.3.tar.gz \
 && echo "${DIGEST} mpc-1.0.3.tar.gz" | sha256sum -c - \
 && tar -xf mpc*.tar.gz \
 && rm mpc*.tar.gz \
 && pushd mpc* \
 && ./configure --prefix=/freebsd --with-gnu-ld --enable-static --enable-shared \
 ...
 && popd \
 && rm -rf mpc*
RUN DIGEST='af300723841062db6ae24e38e61aaf4fbf3f6e5d9fd3bf60ebbdbf95db4e9f09' \
 && wget https://ftp.gnu.org/gnu/gcc/gcc-8.1.0/gcc-8.1.0.tar.gz \
 && echo "${DIGEST} gcc-8.1.0.tar.gz" | sha256sum -c - \
 && tar -xf gcc*.tar.gz \
 && rm gcc*.tar.gz \
 && pushd gcc* \
 && mkdir build \
